...
class RecommendationService {
    ...
    embeddingService;
    vectorService;
    logger = (0, logger_1.createLogger)("RecommendationService");
    ...
    async getRecommendations(diary, watchlist, highlyRatedMovies, subscribedServices, countryCode) {
        ...
        const highlyRatedMovieEmbeddings = [];
        ...
        if (highlyRatedMovieEmbeddings.length === 0) {
            console.log("No embeddings found for highly rated movies. Cannot generate recommendations based on taste profile.");
            return [];
        }
        // Average the embeddings to create a taste profile embedding
        const tasteProfileEmbedding = highlyRatedMovieEmbeddings[0].map((_, i) => highlyRatedMovieEmbeddings.reduce((sum, vec) => sum + vec[i], 0) /
        ...
        const chromaResults = await this.vectorService.queryCollection(this.config.chromaDB.collectionName, tasteProfileEmbedding, this.config.recommendations.chromaTopK);
        const candidateMovieIds = new Set();
        const chromaScoredCandidates = [];
        if (chromaResults && chromaResults.ids && chromaResults.ids.length > 0) {
            for (let i = 0; i < chromaResults.ids[0].length; i++) {
                ...
                if (!movieDetails)
                    continue;
                if (!watchedMovieNames.has(movieDetails.title.toLowerCase()) &&
        ...
        console.log(`Candidates after semantic search and filtering: ${chromaScoredCandidates.length}`);
        ...
        for (const candidate of chromaScoredCandidates) {
            ...
            if (movieCredits && movieCredits.crew) {
                const movieDirectors = new Set(movieCredits.crew
            ...
            if (movieCredits && movieCredits.crew) {
                const movieWriters = new Set(movieCredits.crew
            ...
            if (movieDetails && movieDetails.overview) {
                const overviewWords = new Set(movieDetails.overview
                    .toLowerCase()
                    .split(/\W+/)
            ...
            finalScoredCandidates.push({
                ...candidate,
                score: candidate.score * this.config.recommendations.semanticWeight * 10 +
        ...
        console.log(`Final scored candidates with score > 0: ${scoredCandidates.length}`);
    ...
    async buildTasteProfile(highlyRatedMovies) {
    ...
    async ensureMovieEmbeddingsPopulated(movies) {
        const errors = [];
        let successCount = 0;
        this.logger.info(`Processing embeddings for ${movies.length} movies`);
        if (!this.embeddingService) {
            this.logger.warn("EmbeddingService not initialized. Cannot populate movie embeddings.");
            return;
        }
        const movieEmbeddingsCollection = await this.vectorService.getOrCreateCollection("movie_embeddings");
        ...
        for (const movie of movies) {
            ...
            try {
                ...
                errors.push({
                    movie: movieName,
                    error: error instanceof Error ? error.message : new Error(String(error)),
                });
        ...
        if (errors.length > movies.length * 0.3) {
            this.logger.error(`High failure rate: ${errors.length}/${movies.length} movies failed`, {
                sampleErrors: errors.slice(0, 3).map((e) => e.movie),
            });
            throw new errors_1.EmbeddingError(`Failed to process ${errors.length} out of ${movies.length} movies. Aborting.`, {
        ...
        if (errors.length > 0) {
            this.logger.warn(`Some movies failed to embed: ${errors.length}/${movies.length}`, {
                sampleFailures: errors.slice(0, 5).map((e) => ({
                    movie: e.movie,
                    error: e.error.message,
                })),
    ...
    async ensureMovieEmbedding(movie) {
        if (!this.embeddingService) {
        ...
        if (movieDetails && movieDetails.overview) {
            const embedding = await this.embeddingService.generateEmbedding(movieDetails.overview);
            await this.vectorService.addDocument("movie_embeddings", movieId, embedding, movieDetails.overview, {
                title: movieDetails.title,
    ...
    async populateMoviePool(collection, pagesToFetch = 1 // Fetch 1 page to keep it fast, but from a random category
    ) {
        if (!this.embeddingService) {
            console.warn("EmbeddingService not initialized. Cannot populate movie pool.");
            return;
        }
        ...
        console.log(`Populating ChromaDB with new movie embeddings (fetching ${pagesToFetch} page(s) from a random category)...`);
        let newMovies = [];
        ...
        for (const movie of newMovies) {
            try {
                ...
                console.error(`Error populating embedding for new movie ${movie.title}:`, error);
            }
        }
        console.log("Finished populating new movie embeddings.");
    }
    async getRandomRecommendationWithDetails(highlyRatedMovies, diary, watchlist) {
        ...
        while (!randomMovie && attempts < maxAttempts) {
            ...
            if (response.results && response.results.length > 0) {
                const availableMovies = response.results.filter((movie) => !watchedMovieNames.has(movie.title.toLowerCase()) &&
        ...
        if (movieDetails && movieDetails.genres && tasteProfile.genres.size > 0) {
            ...
            if (matchedGenres.length > 0) {
                reasons.push(`It's in your favorite genres: ${matchedGenres
                    .slice(0, 3)
                    .join(", ")}.`);
            }
        }
        ...
        if (movieCredits && movieCredits.cast && tasteProfile.actors.size > 0) {
            ...
            if (matchedActors.length > 0) {
                reasons.push(`It features actors you like: ${matchedActors
                    .slice(0, 2)
                    .join(", ")}.`);
            }
        }
        ...
        if (movieCredits && movieCredits.crew && tasteProfile.directors.size > 0) {
            const movieDirectors = new Set(movieCredits.crew.filter((c) => c.job === "Director").map((c) => c.name));
