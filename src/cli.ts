      ...
      switch (action) {
        ...
        case "Get personalized recommendations": {
          ...
          if (
            ...
            if (
              ...
              console.error(
                "Please set your streaming services first.",
              );
              break;
            }
        ...
        case "List movies watched in a specific year": {
          ...
          const moviesInYear = diaryData.filter(
            (entry) =>
              new Date(entry.WatchedDate).getFullYear() === parseInt(year),
          );
        ...
        case "Find where to watch a movie": {
          ...
          try {
            ...
            console.log(
              `Found movie: ${movie.title} (${new Date(
                movie.release_date,
              ).getFullYear()})`,
            );
        ...
        case "Suggest a random movie to watch": {
          ...
          if (!suggestionFound) {
            console.log(
              "Could not find a suitable movie from your watchlist.",
            );
          }
          break;
        }
        ...
        case "List available streaming services": {
          try {
            ...
            if (providers && providers.length > 0) {
              console.log(
                `Available streaming services in ${countryCode.toUpperCase()}:`,
              );
              ...
              console.log(
                `Could not find any streaming services for country code: ${countryCode.toUpperCase()}`,
              );
            }
          } catch (e) {
                    ...
                    if (
                      ...
                      if (subscribedProviderNames) {
                        subscribedMovies.push(
                          `- ${movieTitle}: Available on your services (${subscribedProviderNames}). Watch here: ${countryProviders.link}`,
                        );
                      } else {
                        ...
                        otherAvailableMovies.push(
                          `- ${movieTitle}: Available on ${otherProviderNames}. Watch here: ${countryProviders.link}`,
                        );
                      }
                    } else {
                      unavailableMovies.push(
                        `- ${movieTitle}: Not available for streaming in your country.`,
                      );
                    }
                  } catch {
                    if (errorCount === 0) {
                      providersSpinner.fail(
                        `- ${movieTitle}: Error finding watch providers. Further errors will be suppressed.`,
                      );
                    }
                    errorCount++;
                    unavailableMovies.push(
