...
export class RecommendationService {
  ...
  async getRecommendations(
    ...
    console.log(
      "Your top directors:",
      [...tasteProfile.directors.entries()]
        .sort((a, b) => b[1] - a[1])
        .slice(0, 5)
        .map((entry) => entry[0]),
    );
    ...
    const filteredCandidates = candidates.filter(
      (c) =>
        !watchedMovieNames.has(c.title.toLowerCase()) &&
        !watchlistMovieNames.has(c.title.toLowerCase()),
    );
    console.log(
      `Candidates after filtering watched/watchlist: ${filteredCandidates.length}`,
    );

    const scoredCandidatesPromises = filteredCandidates.map(
      async (candidate) => {
        let score = 0;
        ...
        const movieCredits: TMDbCredits =
          await this.tmdbService.getMovieCredits(candidate.id);
    ...
    const resolvedScoredCandidates: ScoredMovieCandidate[] = await Promise.all(
      scoredCandidatesPromises,
    );
  ...
  private async getCandidateMovies(): Promise<TMDbMovie[]> {
    let allCandidates: TMDbMovie[] = [];
    // Fetch multiple pages of popular movies for a larger pool of candidates
    for (let page = 1; page <= 5; page++) {
      // Fetching 5 pages for now (up to 100 movies)
