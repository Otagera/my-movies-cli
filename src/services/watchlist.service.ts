...
export class WatchlistService {
  ...
  public async checkForAvailabilityChanges(): Promise<string[]> {
    ...
    for (const entry of watchlist) {
      ...
      if (movie) {
        ...
        const isAvailable = countryProviders?.flatrate?.some((p: any) =>
          this.subscribedServices.includes(p.provider_name.toLowerCase())
        );

        currentAvailability[movieName] = {
          isAvailable: !!isAvailable,
          providers: isAvailable ? countryProviders.flatrate.map((p: any) => p.provider_name) : [],
          link: isAvailable ? countryProviders.link : undefined,
        };

        if (previousAvailability[movieName]?.isAvailable !== currentAvailability[movieName].isAvailable) {
  ...
  private async getPreviousAvailability(): Promise<WatchlistAvailability> {
    const availability = await this.cacheService.get<WatchlistAvailability>('watchlist-availability');
    return availability || {};
  }
}
